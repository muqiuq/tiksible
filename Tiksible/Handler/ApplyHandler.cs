...
namespace Tiksible.Handler
{
    public class ApplyHandler : BaseHandler, IHandler
    {
        ...
        {
            var applyCommand = new Command("apply", "compare remote config with template and apply diff");
            ...
            AddCredHostsDefaultArgument(applyCommand, out var credOption, out var hostsOption, out var debugOption);

            applyCommand.SetHandler(async (string rscFilename, bool write, string credentialsFileName, string hostsFileName, bool debug) =>
            {
                await HandleApply(rscFilename, write, credentialsFileName, hostsFileName, debug);
            }, rscFilenameArg, writeOption, credOption, hostsOption, debugOption);

            return applyCommand;
        }


        private async Task HandleApply(string rscFilename, bool write, string credentialsFileName, string hostsFileName, bool debug)
        {
